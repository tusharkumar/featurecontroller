...
double diff_time(struct timeval start, struct timeval end) {
	...
	diff_val = (double)(diff.tv_sec) + (double)(diff.tv_usec)/1000000;
	printf(" diff = %lf\n", diff_val);
...
void putseq()
{
  ...
  if(fp_plotfile == 0) { //Tushar
	  fp_plotfile = fopen(plot_filename, "w");
	  if(fp_plotfile == NULL) {
		  fprintf(stderr, "Could not open plot file for writing: %s\n", plot_filename);
		  exit(1);
	  }
  }
  ...
  for (i=0; i<nframes; i++)
  {
    struct timeval start = get_curr_timeval(); //Tushar
    struct timeval end; //Tushar
	double diff; //Tushar

    //opp_frame_enter(frame_name_to_id[fMAIN]); //Tushar
    fmain_fractional_enter(); //Tushar
    ...
    //Opp_ExecTime_t exec_time = opp_frame_exit_complete(frame_name_to_id[fMAIN]); //Tushar
    fmain_fractional_exit(); //Tushar
    //fprintf(stderr, "SRT main-frame (putseq loop) frame-time = %lf\n", exec_time);
    print_srt_stats_for_frame(fMAIN); //Tushar
    print_srt_stats_for_execframe(efCHOOSE_SEARCH_WINDOW); //Tushar

    end = get_curr_timeval(); //Tushar
	diff = diff_time(start, end); //Tushar

	fprintf(fp_plotfile, "%d \t%lf \t%c \t%d\n", i, diff, //Tushar
			(pict_type == I_TYPE ? 'I' :
			 (pict_type == P_TYPE ? 'P' :
			  (pict_type == B_TYPE ? 'B' : 'X'))), srt_model_choice
	);
	fflush(NULL); //Tushar
